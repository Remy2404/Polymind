...
class MockCollection:
    ...
    def update_one(self, query, update, *args, **kwargs):
        ...
        return type('UpdateResult', (), {"acknowledged": True, "matched_count": 1, "modified_count": 1})
...
def get_database(max_retries: int = 3, retry_interval: float = 1.0) -> Tuple[Optional[Database], Optional[MongoClient]]:
    """
    ...
    if not mongodb_uri:
        ...
        if not mongodb_uri:
            logger.error("No MongoDB connection string found in any environment variable")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(f"Database connection attempt {attempt+1} failed: {str(e)}. Retrying in {current_retry_interval:.1f}s...")
...
def _ensure_indexes(db: Database) -> None:
    ...
    try:
        ...
        if 'conversation_history' in db.list_collection_names():
            db.conversation_history.create_index([('user_id', 1), ('timestamp', -1)], background=True)
            
        # Document history indexes
        if 'document_history' in db.list_collection_names():
            db.document_history.create_index([('user_id', 1), ('timestamp', -1)], background=True)
...
async def get_database_async(max_retries: int = 3, retry_interval: float = 1.0) -> Tuple[Optional[Database], Optional[MongoClient]]:
    """
    ...
    return await loop.run_in_executor(None, lambda: get_database(max_retries, retry_interval))
