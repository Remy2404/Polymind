...
class MultiFileProcessor:
    ...
    async def process_multiple_files(self, files: List[Dict], prompt: str) -> Dict[str, Any]:
        """
        ...
        intent, specific_files = self._determine_intent_from_prompt(prompt, categorized_files)
        
        # Process files based on intent
        result = await self._process_files_by_intent(intent, specific_files or categorized_files, prompt)
    ...
    def _categorize_files(self, files: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        for file in files:
            ...
            mime_type = mimetypes.guess_type(filename)[0] or file.get('mime', 'application/octet-stream')
            ...
            elif file_ext in ('.py', '.java', '.cpp', '.c', '.js', '.ts', '.html', '.css', '.php', '.go', '.rb'):
    ...
    def _determine_intent_from_prompt(self, prompt: str, categorized_files: Dict[str, List[Dict]]) -> Tuple[str, Optional[Dict[str, List[Dict]]]]:
        """
        ...
        if any(word in prompt_lower for word in ['compare', 'difference', 'similarities', 'versus', 'vs', 'between']):
            intent = 'compare'
        
        # Check for extraction intent
        elif any(word in prompt_lower for word in ['extract', 'pull out', 'get', 'find', 'retrieve']):
            intent = 'extract'
            
            # Extract what needs to be extracted
            extraction_target = None
            for target in ['text', 'data', 'information', 'content', 'code', 'tables', 'images']:
        ...
        # Check for code-specific intents
            ...
            elif any(word in prompt_lower for word in ['debug', 'fix', 'error', 'issue', 'problem']):
                intent = 'debug_code'
            elif any(word in prompt_lower for word in ['review', 'critique', 'feedback']):
                intent = 'review_code'
    ...
    async def _process_files_by_intent(self, intent: str, categorized_files: Dict[str, List[Dict]], prompt: str) -> Dict[str, Any]:
        """
        ...
        if intent == 'compare':
            ...
            if sum(len(files) for files in file_lists) >= 2:
                if len(file_lists) == 1 and len(file_lists[0]) >= 2:
                    ...
                    processing_tasks.append(self._compare_across_categories(categorized_files, prompt))
        
        elif intent.startswith('extract_'):
            # Extract specific content from files
            extraction_type = intent.split('_')[1]
            for category, files in categorized_files.items():
                processing_tasks.append(self._extract_from_files(files, extraction_type, prompt))
        ...
        elif intent in ['explain_code', 'optimize_code', 'debug_code', 'review_code'] and 'code' in categorized_files:
            # Process code with specific intent
            processing_tasks.append(self._process_code_files(categorized_files['code'], intent, prompt))
    ...
    async def _process_image(self, image_file: Dict, prompt: str) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.error(f"Error processing image {image_file.get('filename', 'unknown')}: {e}")
            return {image_file.get('filename', 'image'): f"Error processing image: {str(e)}"}
    
    async def _process_document(self, document_file: Dict, prompt: str) -> Dict[str, str]:
        ...
        try:
            ...
            if self.document_processor:
                ...
                if isinstance(response, dict) and 'result' in response:
                    ...
                    return {filename: "Processed document successfully but response format was unexpected."}
            else:
                # Fallback to text extraction and analysis
                return {filename: f"Please install document processing capabilities to analyze {file_extension} files."}
        except Exception as e:
            self.logger.error(f"Error processing document {document_file.get('filename', 'unknown')}: {e}")
            return {document_file.get('filename', 'document'): f"Error processing document: {str(e)}"}
    ...
    async def _process_code_file(self, code_file: Dict, prompt: str) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.error(f"Error processing code file {code_file.get('filename', 'unknown')}: {e}")
            return {code_file.get('filename', 'code'): f"Error processing code: {str(e)}"}
    
    async def _process_audio(self, audio_file: Dict, prompt: str) -> Dict[str, str]:
        """Process an audio file"""
        filename = audio_file.get('filename', 'audio')
        return {filename: f"Audio analysis is not fully supported yet. For {filename}, consider uploading as a voice message for transcription."}
    
    async def _process_video(self, video_file: Dict, prompt: str) -> Dict[str, str]:
        """Process a video file"""
        filename = video_file.get('filename', 'video')
        return {filename: f"Video analysis is not fully supported yet. For {filename}, I can only analyze individual frames."}
    
    async def _process_other_file(self, other_file: Dict, prompt: str) -> Dict[str, str]:
        """Process other file types"""
        filename = other_file.get('filename', 'file')
        return {filename: f"This file type is not directly supported for detailed analysis. I can try to analyze as text if you'd like."}
    ...
    async def _compare_across_categories(self, categorized_files: Dict[str, List[Dict]], prompt: str) -> Dict[str, str]:
    ...
    async def _extract_from_files(self, files: List[Dict], extraction_type: str, prompt: str) -> Dict[str, str]:
        """Extract specific content from files"""
        results = {}
        ...
        for file in files:
            ...
            try:
                ...
                if mime_type.startswith('image/'):
                    ...
                    extraction = await self.gemini_api.analyze_image(file_data, enhanced_prompt)
                else:
                    ...
                    try:
                        ...
                        extraction_prompt = f"{enhanced_prompt}\n```\n{content[:4000]}\n```"
                        extraction = await self.gemini_api.generate_response(extraction_prompt)
                    except:
                        extraction = f"Could not extract {extraction_type} from binary file {filename}"
                
                results[filename] = extraction or f"No {extraction_type} found in {filename}"
    ...
    async def _translate_files(self, files: List[Dict], prompt: str) -> Dict[str, str]:
        ...
        for file in files:
            ...
            try:
                ...
                translation = await self.gemini_api.generate_response(translation_prompt)
    ...
    def _extract_target_language(self, prompt: str) -> Optional[str]:
        ...
        for lang_key, lang_value in languages.items():
            if f"to {lang_key}" in prompt_lower or f"into {lang_key}" in prompt_lower or f"in {lang_key}" in prompt_lower:
                return lang_value
        
        return None
    
    async def _process_code_files(self, code_files: List[Dict], intent: str, prompt: str) -> Dict[str, str]:
    ...
    async def _get_file_content_or_description(self, file: Dict) -> str:
        ...
        if mime_type.startswith('image/'):
            ...
            try:
                description = await self.gemini_api.analyze_image(file_data, "Describe this image in detail")
