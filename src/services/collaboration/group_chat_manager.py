...
class GroupChatManager:
    ...
    async def initialize_group(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> Dict[str, Any]:
        """Initialize a group for enhanced collaboration"""
        chat = update.effective_chat
        user = update.effective_user
        
        if not chat or chat.type not in [ChatType.GROUP, ChatType.SUPERGROUP]:
            return {"success": False, "error": "This feature is only available in groups"}
        ...
        try:
            ...
            if member.status not in [ChatMember.ADMINISTRATOR, ChatMember.OWNER]:
                return {"success": False, "error": "Only group administrators can initialize enhanced features"}
        ...
        await self._log_group_activity(
            group_id, user_id, "group_initialized", 
            {"session_id": session_info.get("session_id")}
        )
        
        return {"success": True, "group_info": group_info, "session": session_info}
    
    async def handle_group_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> Dict[str, Any]:
        ...
        if group_id not in self.active_groups:
            return {"success": False, "error": "Group not initialized for enhanced features"}
        ...
        if message_text.startswith('/') or self._is_bot_mentioned(message_text, context):
        ...
        await self._check_collaboration_triggers(group_id, user_id, message_text, context)
    ...
    async def _process_group_message_intelligently(
        ...
        intent_analysis = await self._analyze_message_intent(message_text, message_context)
    ...
    async def _handle_group_ai_interaction(
        ...
        if message_text.startswith('/'):
            response = await self._handle_group_command(group_id, user_id, message_text, context)
        else:
    ...
    async def _handle_group_command(
        ...
        if base_command == "/groupsummary":
            ...
            note_content = " ".join(command_parts[1:]) if len(command_parts) > 1 else None
            return await self._create_collaborative_note(group_id, user_id, note_content, context)
        
        elif base_command == "/grouptasks":
            return await self._show_group_tasks(group_id, context)
        
        elif base_command == "/startdiscussion":
            topic = " ".join(command_parts[1:]) if len(command_parts) > 1 else "General Discussion"
    ...
    async def _generate_group_summary(self, group_id: str, context) -> Dict[str, Any]:
        ...
        try:
            ...
            activity_summary = await self.conversation_manager.memory_manager.get_group_activity_summary(group_id)
            ...
            summary_text += f"• Total messages: {activity_summary.get('total_messages', 0)}\n"
            summary_text += f"• Active participants: {activity_summary.get('active_users', 0)}\n"
            ...
            # Recent highlights
            important_messages = [msg for msg in recent_memory if msg.get('importance', 0) > 0.7]
            ...
            if group_id in self.conversation_manager.collaborative_contexts:
                collab_context = self.conversation_manager.collaborative_contexts[group_id]
                summary_text += f"🤝 **Collaboration Status:**\n"
                summary_text += f"• Active notes: {len(collab_context.get('notes', []))}\n"
                summary_text += f"• Action items: {len(collab_context.get('action_items', []))}\n"
                summary_text += f"• Decisions made: {len(collab_context.get('decisions', []))}\n"
            
            # Create summary keyboard
            summary_keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("📝 Export Summary", callback_data=f"export_summary_{group_id}"),
                    InlineKeyboardButton("📊 Detailed Stats", callback_data=f"detailed_stats_{group_id}")
                ],
                [
                    InlineKeyboardButton("🔍 Smart Search", callback_data=f"smart_search_{group_id}"),
                    InlineKeyboardButton("🤝 Collaboration Hub", callback_data=f"collab_hub_{group_id}")
                ]
            ])
    ...
    async def _create_collaborative_note(
        ...
        if not note_content:
            ...
            prompt_keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("📝 Add Note", callback_data=f"add_note_{group_id}_{user_id}")],
                [
                    InlineKeyboardButton("📋 Meeting Notes", callback_data=f"note_meeting_{group_id}_{user_id}"),
                    InlineKeyboardButton("💡 Idea", callback_data=f"note_idea_{group_id}_{user_id}")
                ],
                [
                    InlineKeyboardButton("✅ Action Item", callback_data=f"note_action_{group_id}_{user_id}"),
                    InlineKeyboardButton("🎯 Decision", callback_data=f"note_decision_{group_id}_{user_id}")
                ]
            ])
        ...
        note_keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("👍 Helpful", callback_data=f"note_react_helpful_{group_id}"),
                InlineKeyboardButton("💬 Discuss", callback_data=f"note_discuss_{group_id}")
            ],
            [InlineKeyboardButton("📚 View All Notes", callback_data=f"view_notes_{group_id}")]
        ])
    ...
    async def _start_group_discussion(
        ...
        discussion_keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("🙋‍♂️ Join Discussion", callback_data=f"join_disc_{discussion_id}"),
                InlineKeyboardButton("📊 Discussion Stats", callback_data=f"disc_stats_{discussion_id}")
            ],
            [
                InlineKeyboardButton("📝 Add Point", callback_data=f"disc_point_{discussion_id}"),
                InlineKeyboardButton("🏁 End Discussion", callback_data=f"end_disc_{discussion_id}")
            ]
        ])
        ...
        return {"success": True, "discussion_started": True, "discussion_id": discussion_id}
    ...
    async def _perform_group_smart_search(
        ...
        if not query:
            ...
            search_keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("🔍 Search Messages", callback_data=f"search_messages_{group_id}"),
                    InlineKeyboardButton("📝 Search Notes", callback_data=f"search_notes_{group_id}")
                ],
                [
                    InlineKeyboardButton("👥 Search by User", callback_data=f"search_user_{group_id}"),
                    InlineKeyboardButton("📅 Search by Date", callback_data=f"search_date_{group_id}")
                ],
                [
                    InlineKeyboardButton("🎯 Search Topics", callback_data=f"search_topics_{group_id}"),
                    InlineKeyboardButton("📊 Search Stats", callback_data=f"search_stats_{group_id}")
                ]
            ])
        ...
        # Perform the search
        try:
            search_results = await self.conversation_manager.memory_manager.get_relevant_memory(
                "", query, limit=10, is_group=True, group_id=group_id, include_group_knowledge=True
            )
            ...
            for i, result in enumerate(search_results[:5], 1):
                ...
                results_text += f"💬 {content[:150]}{'...' if len(content) > 150 else ''}\n\n"
            
            # Create results keyboard
            results_keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("📊 More Results", callback_data=f"more_results_{group_id}_{query}"),
                    InlineKeyboardButton("💾 Save Search", callback_data=f"save_search_{group_id}_{query}")
                ],
                [InlineKeyboardButton("🔍 New Search", callback_data=f"new_search_{group_id}")]
            ])
            ...
            return {"success": True, "results_found": True, "result_count": len(search_results)}
    ...
    def _create_group_welcome_message(self, group_info: Dict[str, Any]) -> str:
        ...
        message += "• 📝 **Collaborative Notes** - Create shared notes and action items\n"
    ...
    def _create_group_setup_keyboard(self, group_id: str) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup([
            [
                InlineKeyboardButton("⚙️ Group Settings", callback_data=f"group_settings_{group_id}"),
                InlineKeyboardButton("👥 Member Roles", callback_data=f"member_roles_{group_id}")
            ],
            [
                InlineKeyboardButton("🧠 AI Personality", callback_data=f"ai_personality_{group_id}"),
                InlineKeyboardButton("📊 Features Setup", callback_data=f"features_setup_{group_id}")
            ],
            [
                InlineKeyboardButton("🎓 Tutorial", callback_data=f"group_tutorial_{group_id}"),
                InlineKeyboardButton("✅ Setup Complete", callback_data=f"setup_complete_{group_id}")
            ]
        ])
    ...
    async def _analyze_message_intent(self, message_text: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif any(t in message_lower for t in ["task", "todo", "deadline", "action", "assign"]):
            intent["type"] = "task_related"
            intent["needs_ai_response"] = False
            intent["priority"] = "medium"
        
        # Check for decision-making
        elif any(d in message_lower for d in ["decide", "choice", "option", "vote", "opinion"]):
            intent["type"] = "decision_making"
            intent["needs_ai_response"] = True
            intent["priority"] = "high"
        
        # Check for information sharing
        elif any(i in message_lower for i in ["fyi", "info", "update", "news", "share"]):
    ...
    async def _get_current_group_context(self, group_id: str) -> Dict[str, Any]:
        ...
        context = {
            ...
            "ongoing_discussions": len([d for d in self.active_discussions.values() 
    ...
    async def _update_group_knowledge_base(self, group_id: str, message_text: str, intent: Dict[str, Any]) -> None:
        ...
        if intent["type"] in ["information_sharing", "decision_making"] and len(message_text) > 50:
            
            # Simple keyword extraction (can be enhanced with NLP)
            keywords = []
            important_words = ["project", "deadline", "meeting", "decision", "important", "remember"]
            ...
            if keywords:
                ...
                self.knowledge_base[group_id][key] = self.knowledge_base[group_id][key][-20:]
    
    async def _log_group_activity(self, group_id: str, user_id: int, activity_type: str, data: Dict[str, Any]) -> None:
    ...
    async def _update_group_activity(self, group_id: str, user_id: int, message_text: str) -> None:
        """Update real-time group activity"""
        
        await self._log_group_activity(
            group_id, user_id, "message_sent", 
            {"message_length": len(message_text), "timestamp": datetime.now().isoformat()}
        )
    ...
    async def _check_collaboration_triggers(
        ...
        if suggestions and len(suggestions) <= 2:  # Don't overwhelm
            suggestion_keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton(suggestion, callback_data=f"collab_suggest_{group_id}_{i}")]
