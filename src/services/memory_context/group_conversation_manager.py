...
class GroupConversationManager:
    ...
    def __init__(self):
        ...
        self.mention_tracker: Dict[int, Dict[int, int]] = defaultdict(lambda: defaultdict(int))
        ...
        self.sentiment_patterns = {
            ...
            "negative": ["bad", "terrible", "hate", "awful", "disappointing", "frustrated"],
            "question": ["?", "how", "what", "when", "where", "why", "which"],
            "urgent": ["urgent", "asap", "emergency", "important", "critical", "help"]
        }
        
        self.topic_keywords = {
            "technical": ["code", "bug", "feature", "development", "programming", "api"],
            "planning": ["meeting", "schedule", "deadline", "plan", "timeline", "roadmap"],
            "general": ["chat", "discussion", "talk", "conversation", "idea", "thought"],
            "support": ["help", "issue", "problem", "question", "assistance", "trouble"]
        }
        
    async def initialize_group(self, group_id: int, group_name: str, group_type: str = "group") -> GroupConversationState:
        ...
        try:
            if group_id not in self.group_states:
                ...
                logger.info(f"Initialized group conversation for {group_name} ({group_id})")
    ...
    async def add_participant(self, group_id: int, user_id: int, username: str, 
                            first_name: str, last_name: str = None, role: str = "member") -> bool:
        ...
        try:
            ...
            logger.error(f"Error adding participant {user_id} to group {group_id}: {str(e)}")
            return False

    async def process_group_message(self, group_id: int, user_id: int, username: str, 
    ...
    async def _analyze_message(self, message: GroupMessage, group_state: GroupConversationState):
        ...
        importance_factors = [
            ...
            1.0 if message.sentiment == "urgent" else 0.0,  # Urgent messages are important
            0.3 if message.reply_to else 0.0,  # Replies show engagement
            min(len(message.content) / 100, 0.3)  # Longer messages might be more important
        ]
        message.importance_score = min(sum(importance_factors), 1.0)

    async def get_group_context(self, group_id: int, max_messages: int = 20) -> Dict[str, Any]:
    ...
    async def _generate_conversation_summary(self, group_state: GroupConversationState) -> str:
        ...
        try:
            ...
            summary_parts.append(f"Active participants: {', '.join(list(participants)[:5])}")
    ...
    async def _update_conversation_context(self, group_id: int, message: GroupMessage):
        ...
        try:
            ...
            group_state.active_threads = [
                thread for thread in group_state.active_threads
    ...
    async def get_participant_insights(self, group_id: int, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            user_messages = [msg for msg in group_state.messages if msg.user_id == user_id]
            
            topics = defaultdict(int)
            sentiments = defaultdict(int)
            mentions_given = self.mention_tracker[user_id]
            mentions_received = sum(1 for msg in group_state.messages if user_id in msg.mentions)
    ...
    async def _create_group_record(self, group_id: int, group_name: str, group_type: str):
        ...
        try:
            ...
            await self.db_connection.execute_query(
                query, 
                (group_id, group_name, group_type, datetime.now(), settings)
            )
    ...
    async def _save_participant(self, group_id: int, participant: GroupParticipant):
        ...
        try:
            ...
            await self.db_connection.execute_query(
                query,
                (group_id, participant.user_id, participant.username, 
                 participant.first_name, participant.last_name, participant.role,
                 participant.join_date, json.dumps(participant.preferences))
            )
    ...
    async def _save_message(self, group_id: int, message: GroupMessage):
        ...
        try:
            ...
            await self.db_connection.execute_query(
                query,
                (message.message_id, group_id, message.user_id, message.username,
                 message.content, message.timestamp, message.message_type, message.reply_to,
                 json.dumps(message.mentions), json.dumps(message.hashtags),
                 message.sentiment, message.topic, message.importance_score,
    ...
    async def get_group_statistics(self, group_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            total_messages = len(group_state.messages)
            active_participants = len([p for p in group_state.participants.values() if p.is_active])
