...
class ConversationManager:
    ...
    async def get_conversation_history(
        self, user_id: int, max_messages: int = 10, model: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        Args:
            ...
            model: Optional model ID to get model-specific history
        ...
        if model:
            ...
            if history:
                self.logger.info(
                    f"Retrieved {len(history)} message(s) from model-specific history for {model}"
                )
                return history

        # Fall back to default history if model is not specified or model-specific history is empty
        return await self.model_history_manager.get_history(
