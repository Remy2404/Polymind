...
class EnhancedIntentDetector:
    ...
    def __init__(self):
        ...
        self.intent_patterns = {
            ...
            CommandIntent.GENERATE_DOCUMENT: {
                ...
                'keywords': ['document', 'report', 'article', 'paper', 'write', 'create', 'generate'],
                'priority': 2
            },
            ...
            CommandIntent.EDUCATIONAL: {
                ...
                'keywords': ['how', 'what', 'why', 'explain', 'tutorial', 'guide', 'difference'],
                'priority': 3
            },
            ...
            CommandIntent.CODING: {
                ...
                'keywords': ['code', 'function', 'script', 'programming', 'debug', 'python', 'javascript'],
                'priority': 2
            }
        }
    ...
    def _detect_intent_fast(self, text: str) -> IntentResult:
        ...
        for intent, config in self.intent_patterns.items():
            ...
            keyword_matches = sum(1 for keyword in config.get('keywords', []) if keyword in text_lower)
            ...
            for pattern in config.get('patterns', []):
                if re.search(pattern, text):
                    ...
                    confidence = min(0.9, 0.5 + (keyword_matches * 0.1) + (config.get('priority', 3) * 0.1))
...
class AICommandRouter:
    ...
    def _extract_prompt_for_document(self, message: str) -> str:
        ...
        cleaned = re.sub(r'(?i)(?:create|generate|write|make)\s+(?:a\s+)?(?:document|report|article|paper)\s+(?:about|on|regarding|for)\s+', '', message, count=1)
    ...
    def _extract_prompt_for_image(self, message: str) -> str:
        ...
        # Simple regex replacement for maximum performance
        cleaned = re.sub(r'(?i)(?:create|generate|draw|make)\s+(?:an?\s+)?(?:image|picture|photo)\s+(?:of|showing|with|depicting)\s+', '', message, count=1)
        cleaned = re.sub(r'(?i)(?:can\s+you\s+|please\s+|could\s+you\s+|i\s+(?:want|need)\s+(?:you\s+to\s+)?)', '', cleaned, count=1)
