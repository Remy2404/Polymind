    ...
    async def process_telegram_message(
        ...
        try:
            ...
            text_prompt = (
                ...
                or message.text
                or message.caption
                or "Please analyze this content."
            )
            ...
            result = await self.gemini_api.process_multimodal_input(
                text_prompt=text_prompt, media_inputs=media_inputs, context=context
            )
        ...
        try:
            ...
            for file_info in files:
                if file_info["type"] == "image":
                    media_inputs.append(
                        create_image_input(
                            file_info["data"], file_info.get("filename", "image")
                        )
                    )
                elif file_info["type"] == "document":
                    media_inputs.append(
                        create_document_input(
                            file_info["data"], file_info.get("filename", "document")
                        )
                    )
                else:
                    # Handle other types
...
async def analyze_mixed_content(
    ...
    result = await gemini_api.process_multimodal_input(
        text_prompt=text, media_inputs=media_inputs if media_inputs else None
    )
