...
class MCPBotIntegration:
    ...
    def __init__(self):
        ...
        # User preferences for MCP usage
    ...
    async def initialize_mcp(self) -> bool:
        ...
        try:
            ...
            if not success:
                self.logger.warning("Failed to load MCP servers - MCP functionality will be disabled")
                self.mcp_manager = None
                return False
            ...
            # Initialize OpenRouter with MCP
    ...
    async def is_mcp_available_for_user(self, user_id: int) -> bool:
        ...
        # Check if user has explicitly enabled/disabled MCP
        if user_id in self.user_mcp_enabled:
            return self.user_mcp_enabled[user_id]

        # Default to enabled if MCP is initialized
        return True
    ...
    async def generate_response_with_mcp(
        ...
        **kwargs
        ...
        try:
            ...
            if response:
                self.logger.info(f"MCP response generated successfully for user {user_id}")
            else:
    ...
    async def get_mcp_status(self) -> Dict[str, Any]:
        ...
        if self.is_initialized and self.mcp_manager:
            ...
            status["tools_count"] = sum(info["tool_count"] for info in server_info.values())
