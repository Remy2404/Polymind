...
class GroupManager:
    ...
    def __init__(self, user_data_manager, conversation_manager):
        ...
        self.participant_cache: Dict[int, Dict[int, GroupParticipant]] = defaultdict(dict)
    ...
    async def handle_group_message(
        ...
        try:
            ...
            metadata = {
                ...
                'context_scope': thread.context_scope.value if thread else 'group_shared',
    ...
    async def _get_conversation_thread(
        ...
        for thread in group_context.threads.values():
            if (thread.last_message_at > recent_cutoff and 
                message.from_user.id in thread.participants):
                thread.last_message_at = datetime.now()
                thread.message_count += 1
                return thread
    ...
    async def _build_enhanced_context(
        ...
        context_parts.append(f"👥 Active Participants: {len(group_context.participants)}")
...
"""
    ...
    async def _update_shared_memory(
        ...
        if len(group_context.recent_messages) > self.max_recent_messages:
            group_context.recent_messages = group_context.recent_messages[-self.max_recent_messages:]
    ...
    async def _extract_and_update_topics(self, group_context: GroupConversationContext, text: str):
    ...
    async def _get_group_settings(self, group_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return group_data.get('group_settings', {
    ...
    async def get_group_analytics(self, group_id: int) -> Dict[str, Any]:
        ...
        total_messages = sum(p.message_count for p in group_context.participants.values())
        active_participants = sum(1 for p in group_context.participants.values() if p.is_active)
        ...
        return {
            ...
            'active_threads': len([t for t in group_context.threads.values() if t.is_active]),
    ...
    async def cleanup_inactive_threads(self, group_id: int):
        ...
        inactive_threads = [
            thread_id for thread_id, thread in group_context.threads.items()
    ...
    async def set_group_context_scope(
        self, 
        group_id: int, 
        scope: ContextScope
    ) -> bool:
        ...
        try:
            if group_id in self.group_contexts:
                ...
                await self.user_data_manager.update_user_data(
                    group_id, 
                    {'group_settings.context_scope': scope.value}
                )
                
                self.logger.info(f"Updated context scope for group {group_id} to {scope.value}")
                return True
        except Exception as e:
