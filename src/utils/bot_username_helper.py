...
class BotUsernameHelper:
    ...
    # No fallback username

    @classmethod
    def get_bot_username(cls, context: Optional[ContextTypes.DEFAULT_TYPE] = None, 
                        bot=None, with_at: bool = True) -> str:
        """
        ...
        Examples:
            >>> get_bot_username(context)  # "@polymind"
            >>> get_bot_username(context, with_at=False)  # "polymind"
        """
        username = None

        # Try to get username from context first (most reliable)
        ...
        # Fallback to direct bot instance
        ...
        # No fallback: if username is still None, return empty string
        if not username:
    ...
    @classmethod
    def is_bot_mentioned(cls, text: str, context: Optional[ContextTypes.DEFAULT_TYPE] = None,
                        bot=None, entities: list = None) -> bool:
        """
        ...
        # Entity-based mention check (more accurate)
        ...
        if entities:
            ...
            for entity in entities:
                ...
                # Check for text_mention entity type
                elif entity.get("type") == "text_mention":
                    user = entity.get("user", {})
                    if (user.get("is_bot", False) and 
                        user.get("username") == bot_username_no_at):
                        mentioned_in_entities = True
                        break
    ...
    @classmethod
    def remove_bot_mention(cls, text: str, context: Optional[ContextTypes.DEFAULT_TYPE] = None,
                          bot=None) -> str:
        """
        ...
        # Remove the mention and clean up whitespace
    ...
    @classmethod
    def get_bot_info_summary(cls, context: Optional[ContextTypes.DEFAULT_TYPE] = None,
                           bot=None) -> dict:
        """
...
# Create a convenient module-level function for easy imports
def get_bot_username(context: Optional[ContextTypes.DEFAULT_TYPE] = None, 
                    bot=None, with_at: bool = True) -> str:
    """Convenience function to get bot username."""
    return BotUsernameHelper.get_bot_username(context, bot, with_at)


def is_bot_mentioned(text: str, context: Optional[ContextTypes.DEFAULT_TYPE] = None,
                    bot=None, entities: list = None) -> bool:
    """Convenience function to check if bot is mentioned."""
    return BotUsernameHelper.is_bot_mentioned(text, context, bot, entities)


def remove_bot_mention(text: str, context: Optional[ContextTypes.DEFAULT_TYPE] = None,
                      bot=None) -> str:
