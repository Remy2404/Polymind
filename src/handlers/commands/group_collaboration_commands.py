...
class GroupCollaborationCommands:
    ...
    async def start_group_session_command(
        ...
        keyboard = [
            [
                InlineKeyboardButton("📝 Add Note", callback_data=f"group_add_note_{group_id}"),
                InlineKeyboardButton("👥 View Participants", callback_data=f"group_participants_{group_id}")
            ],
            [
                InlineKeyboardButton("📊 Group Summary", callback_data=f"group_summary_{group_id}"),
                InlineKeyboardButton("🧠 Shared Memory", callback_data=f"group_memory_{group_id}")
            ],
            [
                InlineKeyboardButton("⚙️ Session Settings", callback_data=f"group_settings_{group_id}"),
                InlineKeyboardButton("🔚 End Session", callback_data=f"group_end_{group_id}")
            ]
        ]
        ...
        self.telegram_logger.log_message(
            user_id, f"Started group session in {group_id} with topic: {topic or 'None'}"
        )
    ...
    async def group_summary_command(
        ...
        group_id = str(update.effective_chat.id) if update.effective_chat.type != "private" else None
        user_id = update.effective_user.id
        ...
        progress_msg = await update.message.reply_text("🔍 Analyzing group conversation...")
        
        try:
            # Get comprehensive group summary
            summary = await self.conversation_manager.memory_manager.get_conversation_summary(
            ...
            # Get group context
            group_context = await self.conversation_manager.get_group_context(group_id, user_id)
            ...
            if activity_summary.get('most_active_user'):
                summary_text += f"• Most active: User {activity_summary['most_active_user']}\n"
            
            # Message types breakdown
            if activity_summary.get('message_types'):
                summary_text += f"\n📋 **Content Types:**\n"
                for msg_type, count in activity_summary['message_types'].items():
                    type_emoji = {"text": "💬", "image": "🖼️", "document": "📄", "voice": "🎤"}.get(msg_type, "📄")
            ...
            keyboard = [
                [
                    ...
                    InlineKeyboardButton("🔍 Detailed Analysis", callback_data=f"detailed_analysis_{group_id}")
                ],
                [
                    InlineKeyboardButton("📝 Add Note", callback_data=f"group_add_note_{group_id}"),
                    InlineKeyboardButton("🔙 Back to Chat", callback_data="dismiss_message")
                ]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await progress_msg.edit_text(summary_text, reply_markup=reply_markup, parse_mode='Markdown')
    ...
    async def group_memory_command(
        ...
        progress_msg = await update.message.reply_text(f"🧠 Searching group memory for: '{query}'...")
        
        try:
            # Search group memory
            relevant_memories = await self.conversation_manager.memory_manager.get_relevant_memory(
                "", query, limit=8, is_group=True, group_id=group_id, include_group_knowledge=True
            )
            ...
            keyboard = [
                [
                    InlineKeyboardButton("🔍 Search Different Term", callback_data=f"memory_search_{group_id}"),
                    InlineKeyboardButton("📊 Memory Stats", callback_data=f"memory_stats_{group_id}")
                ],
                [
                    InlineKeyboardButton("💾 Export Memories", callback_data=f"export_memories_{group_id}"),
                    InlineKeyboardButton("🗑️ Clear Memories", callback_data=f"clear_memories_{group_id}")
                ],
                [
                    InlineKeyboardButton("🔙 Back to Chat", callback_data="dismiss_message")
                ]
            ...
            await progress_msg.edit_text(memory_text, reply_markup=reply_markup, parse_mode='Markdown')
    ...
    async def add_group_note_command(
        ...
        try:
            ...
            confirmation_text += f"👤 **Added by:** {update.effective_user.first_name}\n"
            ...
            keyboard = [
                [
                    InlineKeyboardButton("📝 Add Another Note", callback_data=f"add_another_note_{group_id}"),
            ...
            self.telegram_logger.log_message(
                user_id, f"Added collaborative note to group {group_id}: {note_content[:50]}..."
            )
    ...
    async def group_participants_command(
        ...
        try:
            ...
            participants = await self.conversation_manager.memory_manager.get_group_participants(group_id)
            ...
            if activity_summary.get('total_messages', 0) > 100:
                participants_text += "• High engagement - active collaboration detected\n"
            elif activity_summary.get('total_messages', 0) > 20:
                participants_text += "• Moderate engagement - good team interaction\n"
            else:
                participants_text += "• Getting started - encourage more participation\n"
            
            if len(participants) > 1:
                participants_text += f"• Multi-participant discussions enhance AI context\n"
            
            # Action buttons
            keyboard = [
                [
                    InlineKeyboardButton("📊 Detailed Stats", callback_data=f"detailed_stats_{group_id}"),
                    InlineKeyboardButton("🎯 Engagement Tips", callback_data=f"engagement_tips_{group_id}")
                ],
                [
                    InlineKeyboardButton("🔙 Back to Chat", callback_data="dismiss_message")
                ]
    ...
    async def group_search_command(
        ...
        progress_msg = await update.message.reply_text(f"🔍 Searching group history for: '{search_query}'...")
        ...
        try:
            ...
            results_text += f"**Found:** {len(relevant_memories)} relevant conversations\n\n"
            ...
            keyboard = [
                [
                    InlineKeyboardButton("🔍 Refine Search", callback_data=f"refine_search_{group_id}"),
                    InlineKeyboardButton("📊 Search Analytics", callback_data=f"search_analytics_{group_id}")
                ],
                [
                    InlineKeyboardButton("💾 Save Results", callback_data=f"save_search_{group_id}"),
                    InlineKeyboardButton("🔙 Back to Chat", callback_data="dismiss_message")
                ]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await progress_msg.edit_text(results_text, reply_markup=reply_markup, parse_mode='Markdown')
    ...
    async def _prompt_for_note(self, query, group_id: str):
        ...
        keyboard = [[InlineKeyboardButton("❌ Cancel", callback_data="dismiss_message")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def _show_group_summary_menu(self, query, group_id: str):
        ...
        keyboard = [
            [
                InlineKeyboardButton("📈 Activity Summary", callback_data=f"activity_summary_{group_id}"),
                InlineKeyboardButton("💬 Conversation Summary", callback_data=f"conv_summary_{group_id}")
            ],
            [
                InlineKeyboardButton("📝 Notes Summary", callback_data=f"notes_summary_{group_id}"),
                InlineKeyboardButton("🎯 Topic Analysis", callback_data=f"topic_analysis_{group_id}")
            ],
            [
    ...
    async def _show_memory_interface(self, query, group_id: str):
        ...
        keyboard = [
            [
                InlineKeyboardButton("🔍 Search Memory", callback_data=f"memory_search_{group_id}"),
                InlineKeyboardButton("📊 Memory Stats", callback_data=f"memory_stats_{group_id}")
            ],
            [
                InlineKeyboardButton("📚 Recent Memories", callback_data=f"recent_memories_{group_id}"),
                InlineKeyboardButton("⭐ Important Items", callback_data=f"important_memories_{group_id}")
            ],
            [
                InlineKeyboardButton("🔙 Back", callback_data="dismiss_message")
            ]
        ...
        await query.edit_message_text(interface_text, reply_markup=reply_markup, parse_mode='Markdown')
    ...
    async def _show_participants_details(self, query, group_id: str):
        ...
        try:
            ...
            for participant_id in participants[:5]:  # Show top 5
                ...
                group_context = await self.conversation_manager.get_group_context(group_id, int(participant_id))
            ...
            keyboard = [[InlineKeyboardButton("🔙 Back", callback_data="dismiss_message")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
