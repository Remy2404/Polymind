...
class MediaAnalyzer:
    ...
    async def _analyze_image(self, media: Dict, prompt: str, model: str) -> str:
        ...
        try:
            ...
            self.logger.info(
                f"Using provider {provider.value} for image analysis with model {model}"
            )
            ...
            if provider == Provider.GEMINI:
                ...
                self.logger.warning(
                    f"Unsupported provider {provider.value} for image analysis, falling back to Gemini"
                )
                return await self._analyze_image_gemini(media, prompt)
    ...
    async def _analyze_image_gemini(self, media: Dict, prompt: str) -> str:
        ...
        try:
            ...
            result = await self.gemini_api.process_multimodal_input(
                text_prompt=prompt, media_inputs=[image_input]
            )
    ...
    async def _analyze_image_openrouter(
        self, media: Dict, prompt: str, model: str
    ) -> str:
        ...
        try:
            ...
            if (
                model_config
                and hasattr(model_config, "supports_images")
                and not model_config.supports_images
            ):
                self.logger.warning(
                    f"Model {model} may not support vision, attempting anyway"
                )
            ...
            if result:
                ...
                return (
                    f"Error: No response from OpenRouter vision API for model {model}"
                )
    ...
    async def _analyze_document(self, media: Dict, prompt: str, model: str) -> str:
        ...
        try:
            provider = self._get_provider_for_model(model)
            self.logger.info(
                f"Using provider {provider.value} for document analysis with model {model}"
            )
            ...
            if provider == Provider.GEMINI:
                ...
                self.logger.info(
                    "Document analysis with OpenRouter not yet implemented, falling back to Gemini"
                )
                return await self._analyze_document_gemini(media, prompt, filename)
            else:
                return await self._analyze_document_gemini(media, prompt, filename)
    ...
    async def _analyze_document_gemini(
        self, media: Dict, prompt: str, filename: str
    ) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Error analyzing document {filename} with Gemini: {str(e)}"
            )
