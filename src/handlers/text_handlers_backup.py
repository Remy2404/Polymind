...
# Removed delegation methods - use response_formatter directly
    ...
    def _get_fallback_models(self, primary_model: str) -> List[str]:
        ...
        return fallback_map.get(primary_model, ["deepseek-r1-0528", "llama4_maverick", "gemini"])
    ...
    async def _attempt_with_fallback(
        ...
        for i, model_name in enumerate(models_to_try):
            try:
                self.logger.info(f"Attempting response with model: {model_name} (attempt {i+1}/{len(models_to_try)})")
                
                # Get model handler for current model
                model_handler = model_handler_factory.get_model_handler(model_name)
                ...
                if response and response.strip():
                    if i > 0:  # If we used a fallback model
                        self.logger.info(f"Successfully generated response using fallback model: {model_name}")
    ...
    async def _notify_fallback_usage(self, message, primary_model: str, fallback_model: str):
        ...
        """
        try:
            notification_text = f"⚠️ *{primary_model}* is temporarily unavailable. Using *{fallback_model}* instead."
