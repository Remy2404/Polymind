...
class DocumentSender:
    ...
    def __init__(self):
        ...
        self.supported_extensions = ['.docx', '.pdf', '.txt', '.xlsx', '.pptx', '.doc', '.xls', '.ppt']
    ...
    async def send_document(
        ...
        try:
            ...
            self.logger.info(f"Sending document: {file_path} ({file_size} bytes) to chat {chat_id}")
            ...
            self.logger.info(f"Successfully sent document {file_path} to chat {chat_id}")
            return True
                ...
                if '.docx' in text or '.pdf' in text:
                    ...
                    for line in lines:
                        if any(ext in line for ext in self.supported_extensions):
                            ...
                            for prefix in ['Document created at:', 'File:', 'Path:', 'Saved to:']:
                                if potential_path.startswith(prefix):
                                    potential_path = potential_path[len(prefix):].strip()
                            
                            # Check if file exists
                            if os.path.exists(potential_path):
                                caption = "📄 Document created successfully!"
                                return await self.send_document(
                                    bot, chat_id, potential_path, caption, reply_to_message_id
                                )
    ...
    def find_recent_documents(self, directory: str = ".", max_age_seconds: int = 300) -> list:
        """
