...
class RateLimitStore:
    ...
    def increment_request_count(self, key: str, window_start: float) -> int:
        ...
        if (
            ...
            or self.store[key].get("window_start", 0) < window_start
        ):
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        logger.info(
            f"Rate limiting initialized - Default: {default_requests_per_minute}/min, "
...
class StreamingRateLimitMiddleware(RateLimitMiddleware):
    ...
    def __init__(
        ...
        logger.info(
            f"Streaming rate limiting initialized - "
