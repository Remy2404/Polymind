...
class TelegramBot:
    ...
    async def _keep_alive_task(self):
        ...
        ping_interval = 30  # Even more frequent pings (30 seconds)
        reconnect_attempts = 0
        max_reconnect_attempts = 5

        while True:
            try:
                # 1. Check Telegram API connection with getMe (works with webhook active)
                    ...
                    try:
                        ...
                        async with aiohttp.ClientSession() as session:
                            ...
                            async with session.get(
                                health_url, headers=headers, timeout=5
                            ) as response:
                                if response.status == 200:
                                    response_text = await response.text()
                                    self.logger.debug(
                                        f"Health check passed: {response_text[:100]}..."
                                    )
                                else:
                                    self.logger.warning(
                        ...
                        self.logger.warning(f"Health check HTTP error: {client_err}")
                    except asyncio.TimeoutError:
                        self.logger.warning("Health check timed out")
                    except Exception as health_err:
                        self.logger.warning(f"Health check error: {health_err}")
                else:
                    self.logger.warning(
                        "No WEBHOOK_URL set, skipping self health check"
                    )

                # 3. Do NOT use getUpdates when webhook is active - it will cause a conflict
                # Instead, send a harmless request to Telegram API to keep connection warm
                await self.application.bot.get_my_commands()
                ...
                self.logger.warning(
                    f"Keep-alive check failed: {str(e) or 'Empty error'}"
                )
                ...
                if reconnect_attempts <= max_reconnect_attempts:
                    try:
                        ...
                        self.logger.error(
                            f"Failed to reconnect: {str(reconnect_error) or 'Empty error'}"
                        )
