...
async def test_shared_memory_context():
    ...
    try:
        ...
        if hasattr(message_handlers.text_handler, 'conversation_manager'):
            ...
            logger.info("✅ Voice handler can access TextHandler's conversation_manager")
        else:
            logger.error("❌ Voice handler cannot access TextHandler's conversation_manager")
            return
            
        # Test if they're the same instance
        if text_conv_manager is voice_conv_manager:
            logger.info("✅ SUCCESS: Voice and text handlers share the SAME ConversationManager instance!")
        else:
            logger.error("❌ FAILURE: Voice and text handlers use DIFFERENT ConversationManager instances!")
            
        # Test memory manager sharing
        if hasattr(text_conv_manager, 'memory_manager') and hasattr(voice_conv_manager, 'memory_manager'):
            if text_conv_manager.memory_manager is voice_conv_manager.memory_manager:
                ...
                logger.error("❌ FAILURE: Handlers use different MemoryManager instances!")
