...
async def test_unified_memory_context():
    ...
    try:
        ...
        for i, message in enumerate(history):
            logger.info(f"  Message {i+1}: {message.get('role', 'unknown')} - {message.get('content', '')[:50]}...")
        ...
        if text_found and voice_found:
            logger.info("🎉 SUCCESS: Both text and voice messages are in the same conversation!")
            logger.info("✅ Memory context is unified - voice and text messages share the same history")
        else:
