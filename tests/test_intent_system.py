...
async def test_intent_detection():
    ...
    test_cases = [
        ...
        (
            "Write a comprehensive article on machine learning",
    ...
    for i, (message, expected_intent) in enumerate(test_cases, 1):
        try:
            ...
            expected_routing = expected_intent not in [
                CommandIntent.CHAT,
            ...
            print(
                f"{status} Test {i:2d}: '{message[:40]:<40}' -> {detected_intent.value:<20} (conf: {confidence:.2f}) [Route: {should_route}]"
            )
            ...
            if not routing_correct:
                print(
                    f"         Routing mismatch: expected {expected_routing}, got {should_route}"
                )
...
async def test_prompt_extraction():
    ...
    doc_test_cases = [
        (
            "Create a business plan document for a tech startup",
...
async def test_spacy_fallback():
    ...
    for message, expected in test_cases:
        ...
        print(
            f"  {status} '{message}' -> {detected_intent.value} (conf: {confidence:.2f})"
        )
