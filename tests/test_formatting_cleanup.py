...
--------
    ...
    # Format the response
    formatted = await formatter.format_response(test_response, "test-user", "moonshot-kimi-dev-72b")
    
    print("✨ Formatted response:")
    print(formatted)
    
    # Check if unwanted dashes were removed
    if "---" not in formatted and "-------" not in formatted and "--------" not in formatted:
...
End of response.
    ...
    formatted2 = await formatter.format_response(test_response2, "test-user", "llama-3.3-8b")
    print(f"\n📋 Second test result:")
    print(formatted2)
