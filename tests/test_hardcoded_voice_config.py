...
from src.services.media.voice_config import VoiceConfig, load_config_from_env, voice_config
...
def test_hardcoded_config():
    ...
    actual_quality = quality_obj.value if hasattr(quality_obj, 'value') else str(quality_obj)
...
def test_without_env():
    ...
    lang_vars = [f"VOICE_ENGINE_PREFERENCES_{lang.upper()}" 
