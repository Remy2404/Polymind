...
def analyze_results(results: List[PerformanceResult]) -> Dict[str, Any]:
    ...
    if successful_results:
        ...
        analysis = {
            ...
            "performance_targets": {
                "sub_100ms": sum(1 for rt in response_times if rt < 100) / len(response_times) * 100,
                "sub_50ms": sum(1 for rt in response_times if rt < 50) / len(response_times) * 100,
                "sub_20ms": sum(1 for rt in response_times if rt < 20) / len(response_times) * 100,
            }
        }
...
def print_performance_report(analysis: Dict[str, Any]) -> None:
    ...
    if "response_times" in analysis:
        ...
        print("-" * 40)
        print(f"< 20ms:   {targets['sub_20ms']:.1f}% {'✅' if targets['sub_20ms'] > 80 else '⚠️'}")
        print(f"< 50ms:   {targets['sub_50ms']:.1f}% {'✅' if targets['sub_50ms'] > 90 else '⚠️'}")
        print(f"< 100ms:  {targets['sub_100ms']:.1f}% {'✅' if targets['sub_100ms'] > 95 else '⚠️'}")
        print()
...
async def main():
    ...
    parser.add_argument("--url", required=True, help="Webhook base URL (e.g., http://localhost:8000)")
    parser.add_argument("--token", required=True, help="Bot token")
    parser.add_argument("--requests", type=int, default=100, help="Number of requests")
    parser.add_argument("--concurrency", type=int, default=10, help="Concurrent requests")
    parser.add_argument("--load-test", action="store_true", help="Run sustained load test")
    ...
    async with WebhookPerformanceTester(args.url, args.token) as tester:
        ...
        if args.load_test:
            ...
            print(f"   Mode: Burst test ({args.requests} requests, {args.concurrency} concurrent)")
