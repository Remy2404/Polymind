...
class TestSimplifiedVoiceProcessor:
    ...
    async def test_transcription(self, processor):
        ...
        try:
            ...
            with patch.object(processor, '_transcribe_with_faster_whisper') as mock_transcribe:
                mock_transcribe.return_value = ("Hello world", "en", {"confidence": 0.9})
                
                result = await processor.transcribe(temp_path, engine=SpeechEngine.FASTER_WHISPER)
    ...
    def test_voice_config_integration(self):
        ...
        assert config.quality in [VoiceQuality.FAST, VoiceQuality.BALANCED, VoiceQuality.HIGH]
        
        # Test language detection
        assert config.detect_language is not None
        
        # Test engine selection
        assert config.engine == SpeechEngine.FASTER_WHISPER or config.engine == SpeechEngine.AUTO
